...
from sqlalchemy import (
    Column, String, Text, DateTime, Enum, Float, ForeignKey, Boolean
)
...
class Regulation(Base):
    ...
    status = Column(
        Enum('active','draft','superseded',name='regulation_status'),
        default='active'
    )
...
class Rule(Base):
    ...
    content = Column(Text)
    risk_level = Column(
        Enum('critical','high','medium','low',name='risk_level'),
        default='medium'
    )
...
class Document(Base):
    ...
    storage_tier = Column(
        Enum('hot','warm','cold', name='storage_tier'),
        default='hot'
    )
...
class DocumentRuleMapping(Base):
    ...
    mapped_by = Column(
        Enum('auto','manual','ai_suggested', name='mapped_by'),
...
class ComplianceAlert(Base):
    ...
    priority = Column(
        Enum('urgent','high','medium','low', name='alert_priority')
    )
    message = Column(Text)
    created_at = Column(DateTime, default=datetime.utcnow)
