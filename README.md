...
### 5. Run scheduler service

Запустите фоновый планировщик для регулярного обновления регуляций:
...
Планировщик выполняет три задачи:

- источники **ELI** каждые 6 часов
- источники **RSS** каждый час
- источники **HTML** каждые 24 часа

Параметры `--db-url`, `--config` и `--port` позволяют переопределить
базу данных, путь к файлу с источниками и HTTP-порт.
